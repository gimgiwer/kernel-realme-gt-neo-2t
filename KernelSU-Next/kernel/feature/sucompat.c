...
long ksu_handle_execve_sucompat(const char __user **filename_user, int orig_nr, const struct pt_regs *regs)
{
	...
	/* regs may be NULL when called from the tracepoint path (hook_manager.c) */
	const char __user *const __user *argv_user = regs ?
		(const char __user *const __user *)PT_REGS_PARM2(regs) : NULL;
