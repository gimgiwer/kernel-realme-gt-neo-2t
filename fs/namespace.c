...
static void mnt_free_id(struct mount *mnt)
{
	...
	if (unlikely(mnt->mnt_id >= DEFAULT_SUS_MNT_ID || mnt->mnt.susfs_mnt_id_backup >= DEFAULT_SUS_MNT_ID)) {
		ida_free(&susfs_mnt_id_ida, mnt->mnt.susfs_mnt_id_backup >= DEFAULT_SUS_MNT_ID ?
...
bypass_orig_flow:
	...
	if (susfs_is_current_zygote_domain() && current->susfs_last_fake_mnt_id > 0 &&
...
bypass_orig_flow:
	...
	if (likely(is_current_zygote_domain) && !(flag & CL_ZYGOTE_COPY_MNT_NS) &&
	    current->susfs_last_fake_mnt_id > 0 && mnt->mnt_id < DEFAULT_SUS_MNT_ID) {
		mnt->mnt.susfs_mnt_id_backup = mnt->mnt_id;
		mnt->mnt_id = current->susfs_last_fake_mnt_id++;
	}
#endif
		...
		struct user_namespace *user_ns, struct fs_struct *new_fs)
{
	...
	if (unlikely(current->pid <= 1 || is_global_init(current)))
		is_zygote_pid = false;
#endif
...
void susfs_run_try_umount_for_current_mnt_ns(void) {
	...
	if (unlikely(!current->nsproxy || (current->flags & PF_KTHREAD)))
		return;
