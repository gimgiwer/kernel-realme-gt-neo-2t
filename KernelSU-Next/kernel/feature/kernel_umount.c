...
void ksu_try_umount(const char *mnt, bool check_mnt, int flags, uid_t uid)
{
	...
	/* only umount sus mounts (mnt_id >= DEFAULT_SUS_MNT_ID) */
	struct mount *m = real_mount(path.mnt);
	if (check_mnt && m && m->mnt_id < DEFAULT_SUS_MNT_ID) {
		path_put(&path);
		return;
	}
#endif
...
void susfs_try_umount_all(uid_t uid) {
	...
	/* only umount ksu module overlays */
	ksu_try_umount("/data/adb/modules", true, MNT_DETACH, uid);
